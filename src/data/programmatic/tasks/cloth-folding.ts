...
const data: TaskPageData = {
  slug: "cloth-folding",
  metaTitle: "Cloth Folding Training Data for Robotics | Claru",
  metaDescription:
  ...
  primaryKeyword: "cloth folding training data",
  secondaryKeywords: [
    "cloth folding dataset",
    "deformable manipulation data",
    "robot garment folding demonstrations",
    "fabric manipulation training data",
    "towel folding automation data",
    "textile handling robot dataset"
],
  canonicalPath: "/training-data/cloth-folding",
  h1: "Cloth Folding Task Training Data",
  heroSubtitle:
    "Cloth folding datasets for deformable object manipulation — garment folding, towel manipulation, and fabric handling with dense keypoint tracking, mesh state estimation, and bimanual coordination annotations.",
  breadcrumbs: [
    { label: "Home", href: "/" },
    { label: "Training Data", href: "/training-data" },
    { label: "Cloth Folding", href: "/training-data/cloth-folding" },
  ],
  sections: [
    {
      type: "prose",
      heading: "What Is Robotic Cloth Folding and Why Does Data Matter?",
      paragraphs: [
        "Cloth Folding is a critical robotic manipulation capability with growing demand across industrial automation, logistics, and service robotics. The task requires precise coordination of perception, planning, and control — making high-quality demonstration data essential for training policies that generalize beyond scripted trajectories. Traditional automation approaches rely on hand-coded programs that must be reprogrammed for each product variant or environment change, creating a bottleneck that learning-based approaches can overcome with sufficient training data.",
        "The data challenge in cloth folding is multifaceted. Success depends on precise force control, accurate state estimation, and adaptive behavior in response to environmental variation. Demonstrations must capture not just the nominal execution trajectory but the full range of corrective behaviors, error recovery strategies, and edge cases that arise in real operations. Without this diversity, learned policies overfit to idealized conditions and fail when deployed in the variability of production environments.",
        "Research has consistently shown that multimodal data — combining vision, force/torque sensing, and proprioception — dramatically improves policy performance for cloth folding tasks. Vision-only policies typically achieve 60-75% success rates on contact-rich variants of this task, while multimodal policies reach 85-95% by leveraging force feedback for precise contact state estimation and compliance control. This 15-30 percentage point improvement establishes multimodal demonstration data as essential for production-grade cloth folding systems.",
        "The commercial applications of robotic cloth folding span manufacturing, logistics, healthcare, and service industries. As labor costs rise and workforce availability tightens, automation of cloth folding operations becomes increasingly economically compelling. The primary barrier to adoption is not hardware capability but the availability of training data that captures the full complexity of real-world cloth folding scenarios — including material variation, environmental uncertainty, and the diverse strategies that experienced human operators employ.",
      ],
    },
    {
      type: "stats",
      heading: "Cloth Folding Data by the Numbers",
      stats: [
        { value: "85-95%", label: "Multimodal policy success rate on cloth folding" },
    ...
    },
    {
      ...
      description:
        "Different learning architectures for cloth folding have distinct data requirements. Choose based on your deployment constraints and available sensing.",
      columns: [
        "Approach",
        "Data Volume",
        "Key Modalities",
    ...
    {
      type: "prose",
      heading: "State of the Art in Learned Cloth Folding",
      paragraphs: [
        "Recent advances in robot learning have dramatically improved the capabilities of learned cloth folding policies. Diffusion Policy (Chi et al., 2023) has emerged as a leading architecture for contact-rich manipulation tasks, achieving state-of-the-art results on benchmarks that include cloth folding components. The key advantage of diffusion models for this task is their ability to represent multimodal action distributions — when multiple valid execution strategies exist, the policy generates diverse high-quality actions rather than averaging across strategies.",
        "ACT (Action Chunking with Transformers, Zhao et al., 2023) has shown particular promise for cloth folding tasks that require smooth, temporally coherent trajectories. By predicting action sequences of 10-50 timesteps rather than individual actions, ACT produces the continuous motions needed for contact-rich tasks without the jerky transitions that plague frame-by-frame policies. On real-robot benchmarks involving similar manipulation skills, ACT achieves 85-96% success rates with only 50-100 teleoperated demonstrations.",
        "Foundation models like RT-2 (Brohan et al., 2023) and OpenVLA (Kim et al., 2024) have demonstrated that pretrained vision-language models can be fine-tuned for cloth folding with significantly less task-specific data than training from scratch. RT-2 achieves 60-75% zero-shot success on novel task variants described in natural language, suggesting that internet-scale pretraining provides transferable understanding of physical interactions. However, precision-critical aspects of cloth folding still require task-specific demonstrations to achieve production-grade reliability.",
        "The Open X-Embodiment initiative (Padalkar et al., 2023) has aggregated 970K robot episodes across 60+ datasets, providing a large-scale pretraining corpus that improves downstream performance on cloth folding by 50-100% compared to training on individual datasets. Models pretrained on OXE and fine-tuned with 1K-5K task-specific demonstrations consistently outperform models trained from scratch on 10K demonstrations, establishing the pretrain-then-fine-tune paradigm as the most data-efficient approach for new cloth folding deployments.",
      ],
    },
    {
      type: "prose",
  ...
  faqs: [
    {
      question: "How many demonstrations are needed for cloth folding?",
      answer:
    ...
    {
      question: "Is force/torque data essential for cloth folding demonstrations?",
      answer:
    ...
    {
      question: "Can simulation replace real cloth folding demonstrations?",
      answer:
        "Simulation is effective for pretraining perception and generating diverse configurations, but the sim-to-real gap for contact-rich cloth folding tasks is typically 15-25 percentage points. The optimal approach is simulation pretraining (100K+ episodes) followed by real-world fine-tuning (500-2,000 demonstrations), which outperforms either modality alone. Real demonstrations are essential for capturing material properties, friction variations, and sensor characteristics that simulation approximates poorly.",
    },
    {
  ...
  ctaHeading: "Get a Custom Quote for Cloth Folding Task Data",
  ctaDescription:
    "Tell us about your cloth folding requirements and we will design a data collection plan matched to your specific application and deployment constraints.",
  ...
  dataRequirements: {
    modality: "Multi-view RGB-D + dense keypoint tracking + bimanual proprioception",
    volumeRange: "1K-10K folding demonstrations per garment type",
    temporalResolution: "30 Hz multi-view video, per-fold keypoint annotations, mesh state snapshots",
    keyAnnotations: [
      "Dense cloth keypoint positions (50-200 per garment)",
      "Grasp point and fold line annotations per manipulation step",
      "Cloth mesh state estimation at key frames",
      "Bimanual coordination timing and handoff annotations",
      "Garment type, size, material, and initial configuration",
      "Fold sequence classification and completion percentage"
],
  },
  relevantModels: [
    "FlingBot",
    "ClothFunnels",
    "Diffusion Policy for deformables",
    "SpeedFolding",
    "DextAIRity",
    "VCD (Visual Cloth Dynamics)"
],
  environmentTypes: [
    "Laundry processing facility",
    "Textile manufacturing line",
    "Household service robot workspace",
    "Retail clothing store",
    "Hospital linen service",
    "Research deformable manipulation lab"
],
  keyPapers: [
    {
      id: "chi-diffusion-2023",
  ...
  claruRelevance:
    "Claru provides cloth folding data collection with precision-instrumented workstations featuring calibrated multi-view RGB-D cameras (2 fixed + 1 wrist-mounted, synchronized at 30 Hz), 6-axis force/torque sensing at the wrist (500 Hz, 0.01 N resolution), and proprioceptive recording at 50-100 Hz. Our operators are trained on client-specific cloth folding procedures with qualification protocols before production collection. We systematically vary configurations, conditions, and strategies to maximize diversity. Deliverables include synchronized multi-view video, force profiles, proprioceptive streams, per-episode annotations, and formatted outputs for Diffusion Policy, ACT/ALOHA, RT-2, OpenVLA, Octo, or custom architectures.",
};
