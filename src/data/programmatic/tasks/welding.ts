...
const data: TaskPageData = {
  slug: "welding",
  metaTitle: "Welding Training Data for Robotics | Claru",
  metaDescription:
  ...
  primaryKeyword: "robotic welding training data",
  secondaryKeywords: [
    "welding task dataset",
    "robot welding demonstrations",
    "seam tracking training data",
    "weld quality automation data",
    "torch orientation dataset",
    "welding parameter optimization data"
],
  canonicalPath: "/training-data/welding",
  h1: "Welding Task Training Data",
  heroSubtitle:
    "Welding datasets for robotic fabrication — seam tracking, torch orientation, and weld parameter control with real-time thermal monitoring, bead geometry annotations, and multi-pass trajectory planning.",
  breadcrumbs: [
    { label: "Home", href: "/" },
    { label: "Training Data", href: "/training-data" },
    { label: "Welding", href: "/training-data/welding" },
  ],
  sections: [
    {
      type: "prose",
      heading: "What Is Robotic Welding and Why Does Data Matter?",
      paragraphs: [
        "Welding is a critical robotic manipulation capability with growing demand across industrial automation, logistics, and service robotics. The task requires precise coordination of perception, planning, and control — making high-quality demonstration data essential for training policies that generalize beyond scripted trajectories. Traditional automation approaches rely on hand-coded programs that must be reprogrammed for each product variant or environment change, creating a bottleneck that learning-based approaches can overcome with sufficient training data.",
        "The data challenge in welding is multifaceted. Success depends on precise force control, accurate state estimation, and adaptive behavior in response to environmental variation. Demonstrations must capture not just the nominal execution trajectory but the full range of corrective behaviors, error recovery strategies, and edge cases that arise in real operations. Without this diversity, learned policies overfit to idealized conditions and fail when deployed in the variability of production environments.",
        "Research has consistently shown that multimodal data — combining vision, force/torque sensing, and proprioception — dramatically improves policy performance for welding tasks. Vision-only policies typically achieve 60-75% success rates on contact-rich variants of this task, while multimodal policies reach 85-95% by leveraging force feedback for precise contact state estimation and compliance control. This 15-30 percentage point improvement establishes multimodal demonstration data as essential for production-grade welding systems.",
        "The commercial applications of robotic welding span manufacturing, logistics, healthcare, and service industries. As labor costs rise and workforce availability tightens, automation of welding operations becomes increasingly economically compelling. The primary barrier to adoption is not hardware capability but the availability of training data that captures the full complexity of real-world welding scenarios — including material variation, environmental uncertainty, and the diverse strategies that experienced human operators employ.",
      ],
    },
    {
      type: "stats",
      heading: "Welding Data by the Numbers",
      stats: [
        { value: "85-95%", label: "Multimodal policy success rate on welding" },
    ...
    },
    {
      ...
      description:
        "Different learning architectures for welding have distinct data requirements. Choose based on your deployment constraints and available sensing.",
      columns: [
        "Approach",
        "Data Volume",
        "Key Modalities",
    ...
    {
      type: "prose",
      heading: "State of the Art in Learned Welding",
      paragraphs: [
        "Recent advances in robot learning have dramatically improved the capabilities of learned welding policies. Diffusion Policy (Chi et al., 2023) has emerged as a leading architecture for contact-rich manipulation tasks, achieving state-of-the-art results on benchmarks that include welding components. The key advantage of diffusion models for this task is their ability to represent multimodal action distributions — when multiple valid execution strategies exist, the policy generates diverse high-quality actions rather than averaging across strategies.",
        "ACT (Action Chunking with Transformers, Zhao et al., 2023) has shown particular promise for welding tasks that require smooth, temporally coherent trajectories. By predicting action sequences of 10-50 timesteps rather than individual actions, ACT produces the continuous motions needed for contact-rich tasks without the jerky transitions that plague frame-by-frame policies. On real-robot benchmarks involving similar manipulation skills, ACT achieves 85-96% success rates with only 50-100 teleoperated demonstrations.",
        "Foundation models like RT-2 (Brohan et al., 2023) and OpenVLA (Kim et al., 2024) have demonstrated that pretrained vision-language models can be fine-tuned for welding with significantly less task-specific data than training from scratch. RT-2 achieves 60-75% zero-shot success on novel task variants described in natural language, suggesting that internet-scale pretraining provides transferable understanding of physical interactions. However, precision-critical aspects of welding still require task-specific demonstrations to achieve production-grade reliability.",
        "The Open X-Embodiment initiative (Padalkar et al., 2023) has aggregated 970K robot episodes across 60+ datasets, providing a large-scale pretraining corpus that improves downstream performance on welding by 50-100% compared to training on individual datasets. Models pretrained on OXE and fine-tuned with 1K-5K task-specific demonstrations consistently outperform models trained from scratch on 10K demonstrations, establishing the pretrain-then-fine-tune paradigm as the most data-efficient approach for new welding deployments.",
      ],
    },
    {
      type: "prose",
      heading: "Collection Methodology for Welding Data",
      paragraphs: [
        "Data collection for welding requires a workspace instrumented for high-fidelity demonstration capture. The standard setup includes 2-3 calibrated RGB-D cameras covering the workspace from multiple viewpoints (overhead, angled, wrist-mounted), a 6-axis force/torque sensor at the robot wrist for contact force measurement, and proprioceptive recording from robot joint encoders at 50-100 Hz. Camera placement should ensure continuous visibility of the manipulation target throughout the entire task execution, including during phases where the robot hand may occlude direct overhead views.",
  ...
  faqs: [
    {
      question: "How many demonstrations are needed for welding?",
      answer:
    ...
    {
      question: "Is force/torque data essential for welding demonstrations?",
      answer:
  ...
  ctaHeading: "Get a Custom Quote for Welding Task Data",
  ctaDescription:
    "Tell us about your welding requirements and we will design a data collection plan matched to your specific application and deployment constraints.",
  ...
  dataRequirements: {
    modality: "RGB + thermal imaging + weld current/voltage + torch pose + seam geometry",
    volumeRange: "1K-10K weld passes per joint type",
    temporalResolution: "30 Hz visual, 1000 Hz electrical parameters, per-pass quality annotations",
    keyAnnotations: [
      "Seam geometry and joint type classification",
      "Torch pose trajectory (position, orientation, travel speed)",
      "Weld parameters (current, voltage, wire feed speed, gas flow)",
      "Thermal profile from infrared monitoring",
      "Bead geometry measurements (width, height, penetration)",
      "Weld quality classification (acceptable, porosity, undercut, spatter)"
],
  },
  relevantModels: [
    "Seam tracking neural networks",
    "Weld parameter optimization models",
    "Adaptive welding controllers",
    "Digital twin welding simulators",
    "Vision-guided welding systems",
    "Multi-pass planning algorithms"
],
  environmentTypes: [
    "Automotive body shop",
    "Shipyard fabrication",
    "Structural steel fabrication",
    "Aerospace welding cell",
