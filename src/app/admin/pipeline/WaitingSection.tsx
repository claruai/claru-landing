...
function LeadRow({ lead }: { lead: EnrichedLead }) {
  ...
  function handleMove(column: "respond" | "their-court" | "nurturing" | "cold") {
    setMenuOpen(false);
    startPendingTransition(() => moveLeadToColumn(lead.lead_id, column));
  }

  return (
    <div
      className={`group relative flex items-center justify-between gap-4 rounded-md border border-[var(--border-subtle)] bg-[var(--bg-secondary)] px-4 py-3 transition-colors hover:border-[var(--border-medium)] ${
    ...
    >
      ...
      {/* Thread state chip */}
      <span
        className={`shrink-0 font-mono text-xs capitalize ${stateColor}`}
      >
        {lead.thread_state}
      </span>
        ...
        {menuOpen && (
          <>
            ...
            <div className="absolute right-0 top-7 z-20 min-w-[180px] rounded-md border border-[var(--border-subtle)] bg-[var(--bg-elevated)] py-1 shadow-lg">
              <MenuButton onClick={() => handleMove("respond")}>
                Move to Respond Now
              </MenuButton>
              <MenuButton onClick={() => handleMove("nurturing")}>
                Move to Nurturing
              </MenuButton>
              <MenuButton onClick={() => handleMove("cold")}>
                Move to Cold
              </MenuButton>
            </div>
          </>
...
export function WaitingSection({ leads, filterType }: WaitingSectionProps) {
  const filtered = leads
    .filter((l) => filterType === "all" || l.type === filterType)
    .filter(
      (l) =>
        l.waiting_on === "them" &&
        ["hot", "warm", "negotiating"].includes(l.thread_state)
    )
  ...
  return (
    <section className="mb-8">
      <h2 className="mb-3 font-mono text-xs font-medium uppercase tracking-wider text-[var(--text-muted)]">
        Waiting on Them ({optimisticLeads.length})
      </h2>
...
// Internal row that receives onMove callback (avoids needing optimistic inside LeadRow)
function WaitingLeadRow({
  lead,
  onMove,
}: {
  lead: EnrichedLead;
